...
class Trader:
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        basket_orders = self.basket_trader(state)
        print(self.positions)
        return {
            "PEARLS": pearl_order,
            "BANANAS": banana_order,
            "BERRIES": mayberry_order,
            "PICNIC_BASKET": basket_orders
        }
    ...
    def basket_trader(self, state: TradingState) -> List[Order]:
        '''
        Simple basket strategy, buy/sell at 10000 cutoff
        '''
        orders = []
        if any(i not in state.order_depths.keys() for i in ['PICNIC_BASKET', 'DIP', 'BAGUETTE', 'UKULELE']):
            return orders

        buy_diff = max(state.order_depths['PICNIC_BASKET'].buy_orders.keys()) - \
            max(state.order_depths['DIP'].sell_orders.keys()) * 4 - \
            max(state.order_depths['BAGUETTE'].sell_orders.keys()) * 2 - \
            max(state.order_depths['UKULELE'].sell_orders.keys())
        
        if buy_diff > 450:
            short_price = max(state.order_depths['PICNIC_BASKET'].buy_orders.keys())
            orders.append(Order("PICNIC_BASKET", short_price, -state.order_depths['PICNIC_BASKET'].buy_orders[short_price]))            
        
        sell_diff = min(state.order_depths['PICNIC_BASKET'].sell_orders.keys()) - \
            min(state.order_depths['DIP'].buy_orders.keys()) * 4 - \
            min(state.order_depths['BAGUETTE'].buy_orders.keys()) * 2 - \
            min(state.order_depths['UKULELE'].buy_orders.keys()) 
        
        if sell_diff < 350:
            long_price = min(state.order_depths['PICNIC_BASKET'].sell_orders.keys())
            orders.append(Order("PICNIC_BASKET", long_price, -state.order_depths['PICNIC_BASKET'].sell_orders[long_price]))
        
        print(buy_diff, sell_diff)
        return orders
    ...
    def mayberry_trader(self, book: OrderDepth, time: int) -> List[Order]:
        ...
        if time < 500000:
            ...
            if self.positions['BERRIES'] > -250 and max(book.buy_orders.keys()) > 3900:
